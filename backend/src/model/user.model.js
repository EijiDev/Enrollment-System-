...
const User = {
  async create({
    ...
    const results = await db.query(
      `INSERT INTO users
      (name, email, password, role, contact_number, address, dateofbirth, gender)
  ...
  async updateUser(user_id, updates) {
    ...
    const query = `UPDATE users ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP
  ...
  async deleteUser(user_id) {
    const results = await db.query("DELETE FROM users WHERE user_id = $1 RETURNING user_id", [user_id]);
    return results.rows[0];
  },
